...
| SCANNER_ADAPTER_IGNORE_HARBOR_CREDS | If set to True, the adapter will ignore the dynamic credentials provided by Harbor for each scan and will instead expect that the admin has configured Anchore with registry credentials out-of-band | False |
